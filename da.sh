...
get_network() {
    ...
    local mask=$(echo "$ip_mask    local IFS='.'
...
configure_user() {
    ...
    if [[ -n "$UID" && "$UID" =~ ^[0-9]+$ ]]; then
        ...
        if getent passwd "$UID" > /dev/null; then
            echo "Error: UID $UID is already in use. Creating user without specific UID."
            adduser "$USERNAME"
        else
            # Attempt to create user with specified UID
            if adduser "$USERNAME" --uid "$UID" 2>/dev/null; then
                echo "User $USERNAME created with UID $UID."
            else
                echo "Error: Failed to create user $USERNAME with UID $UID. Creating without specific UID."
                adduser "$USERNAME"
            fi
        fi
    else
        # Create user without specific UID
        echo "No valid UID provided or UID not specified. Creating user with system-assigned UID."
        adduser "$USERNAME"
    fi

    # Set password and configure sudo rights if user creation was successful
    if id "$USERNAME" &> /dev/null; then
        echo "$USERNAME:$PASSWORD" | chpasswd
        echo "$USERNAME ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers
        usermod -aG wheel "$USERNAME"
        echo "User $USERNAME configured with sudo rights."
    else
