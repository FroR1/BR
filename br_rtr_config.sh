...
# BR-RTR Configuration Script
# This script configures network interfaces, nftables, hostname, and timezone for BR-RTR.
# Target distribution: ALT Linux (uses /etc/net/ifaces for network configuration).
# Logs actions to /var/log/br_rtr_config.log.
...
LOG_FILE="/var/log/br_rtr_config.log"
...
log_message "Starting BR-RTR configuration script..."
...
edit_data() {
    while true; do
        clear
        echo "Current Data:"
        echo "1. ISP interface: ${INTERFACE_ISP:-Not set}"
        echo "2. LAN interface: ${INTERFACE_LAN:-Not set}"
        echo "3. IP for ISP: ${IP_ISP:-Not set}"
        echo "4. IP for LAN: ${IP_LAN:-Not set}"
        echo "5. Default gateway: ${DEFAULT_GW:-Not set}"
        ...
        case $choice in
            1) read -p "Enter ISP interface name: " INTERFACE_ISP ;;
            2) read -p "Enter LAN interface name: " INTERFACE_LAN ;;
            3)
                while true; do
                    read -p "Enter IP for ISP (e.g., 172.16.5.2/28): " IP_ISP
                    if validate_ip "$IP_ISP"; then break; else
                        echo "Invalid IP format. Use format like 172.16.5.2/28."
            ...
            4)
                while true; do
                    read -p "Enter IP for LAN (e.g., 192.168.1.1/24): " IP_LAN
                    if validate_ip "$IP_LAN"; then break; else
                        echo "Invalid IP format. Use format like 192.168.1.1/24."
                        read -p "Press Enter to try again..."
                    fi
                done
                ;;
            5) read -p "Enter default gateway (e.g., 172.16.5.1): " DEFAULT_GW ;;
            ...
            7)
                ...
                ;;
            8)
                read -p "Enter ISP interface name: " INTERFACE_ISP
                read -p "Enter LAN interface name: " INTERFACE_LAN
                while true; do
                    read -p "Enter IP for ISP (e.g., 172.16.5.2/28): " IP_ISP
                    if validate_ip "$IP_ISP"; then break; else
                        echo "Invalid IP format. Use format like 172.16.5.2/28."
                ...
                read -p "Enter default gateway (e.g., 172.16.5.1): " DEFAULT_GW
...
configure_interfaces() {
    if [ -z "$IP_ISP" ] || [ -z "$IP_LAN" ] || [ -z "$INTERFACE_ISP" ] || [ -z "$INTERFACE_LAN" ] || [ -z "$DEFAULT_GW" ]; then
        log_message "Error: Interface names, IP addresses, or default gateway not set. Please set them in option 1."
        read -p "Press Enter to continue..."
        return 1
    fi

    # Check if interfaces exist
    for iface in "$INTERFACE_ISP" "$INTERFACE_LAN"; do
    ...
    mkdir -p "/etc/net/ifaces/$INTERFACE_ISP" >> "$LOG_FILE" 2>&1 || { log_message "Error: Failed to create directory for $INTERFACE_ISP."; return 1; }
    echo -e "BOOTPROTO=static\nTYPE=eth\nDISABLED=no\nCONFIG_IPV4=yes" > "/etc/net/ifaces/$INTERFACE_ISP/options" 2>>"$LOG_FILE" || { log_message "Error: Failed to write options for $INTERFACE_ISP."; return 1; }
    echo "$IP_ISP" > "/etc/net/ifaces/$INTERFACE_ISP/ipv4address" 2>>"$LOG_FILE" || { log_message "Error: Failed to set IP for $INTERFACE_ISP."; return 1; }
    echo "default via $DEFAULT_GW" > "/etc/net/ifaces/$INTERFACE_ISP/ipv4route" 2>>"$LOG_FILE" || { log_message "Error: Failed to set default gateway for $INTERFACE_ISP."; return 1; }

    # Configure LAN interface
    mkdir -p "/etc/net/ifaces/$INTERFACE_LAN" >> "$LOG_FILE" 2>&1 || { log_message "Error: Failed to create directory for $INTERFACE_LAN."; return 1; }
    echo -e "BOOTPROTO=static\nTYPE=eth\nDISABLED=no\nCONFIG_IPV4=yes" > "/etc/net/ifaces/$INTERFACE_LAN/options" 2>>"$LOG_FILE" || { log_message "Error: Failed to write options for $INTERFACE_LAN."; return 1; }
    echo "$IP_LAN" > "/etc/net/ifaces/$INTERFACE_LAN/ipv4address" 2>>"$LOG_FILE" || { log_message "Error: Failed to set IP for $INTERFACE_LAN."; return 1; }
    ...
    log_message "Interfaces $INTERFACE_ISP and $INTERFACE_LAN configured."
    read -p "Press Enter to continue..."
}

# Function to configure nftables via /etc/nftables/nftables.nft
configure_nftables() {
    if [ -z "$IP_LAN" ] || [ -z "$INTERFACE_ISP" ]; then
        log_message "Error: LAN IP or ISP interface not set. Please set them in option 1."
        read -p "Press Enter to continue..."
        return 1
    fi
    ...
    # Calculate LAN network for masquerading
    LAN_NETWORK=$(get_network "$IP_LAN") || { log_message "Error: Failed to calculate LAN network."; return 1; }
    log_message "LAN Network: $LAN_NETWORK"
...
while true; do
    ...
    case $choice in
        ...
        0) log_message "Exiting BR-RTR configuration script."; clear; exit 0 ;;
        *) echo "Invalid choice."; read -p "Press Enter to continue..." ;;
    esac
done
